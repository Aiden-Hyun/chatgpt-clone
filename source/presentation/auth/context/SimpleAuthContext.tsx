...
export function SimpleAuthProvider({ children }: AuthProviderProps) {
  ...
  useEffect(() => {
    Logger.info("SimpleAuthProvider: Starting auth initialization");
    let mounted = true;
    ...
    const initializeAuth = async () => {
      try {
        ...
        Logger.debug("SimpleAuthProvider: Initial auth state", {
        ...
        Logger.error("SimpleAuthProvider: Auth initialization error", {
          error,
        });
        if (mounted) {
          setSession(null);
    ...
    const startMonitoring = async () => {
      try {
        const monitorAuthStateUseCase =
          useCaseFactory.createMonitorAuthStateUseCase();
        const monitorResult = await monitorAuthStateUseCase.execute();

        if (monitorResult.success) {
          Logger.info("SimpleAuthProvider: Auth monitoring started");
            ...
            if (mounted) {
              try {
                const getAuthStateUseCase =
                  useCaseFactory.createGetAuthStateUseCase();
                ...
                if (currentResult.success) {
                  ...
                  setSession((prevSession) => {
                    ...
                    if (
                      prevSession &&
                      newSession &&
                      prevSession.userId === newSession.userId
                    ) {
                ...
                Logger.warn("SimpleAuthProvider: Polling error", { error });
              }
            }
          ...
          Logger.warn("SimpleAuthProvider: Failed to start monitoring", {
            error: monitorResult.error,
          });
        }
      } catch (error) {
        Logger.error("SimpleAuthProvider: Monitoring setup error", { error });
      }
    };
