...
export const MessageList: React.FC<MessageListProps> = ({
  ...
  const isNewMessageLoading = useMemo(() => {
    ...
    return (
      ...
      (lastMessage as { state?: string })?.state === "loading"
    );
  }, [messages]);
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < messages.length; i++) {
      const explicitId =
        (messages[i] as { id?: string; _loadingId?: string }).id ??
        (messages[i] as { id?: string; _loadingId?: string })._loadingId;
      if (explicitId) {
        ids[i] = explicitId;
      } else if (!ids[i]) {
        ...
        ? [
            ...messagesWithIds,
            {
              ...
              id: (placeholderIdRef.current ||=
                new IdGenerator().generateMessageId()),
            } as ChatMessage,
          ]
        : messagesWithIds,
