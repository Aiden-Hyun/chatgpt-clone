...
export class SupabaseAuthAdapter {
  async signIn(email: string, password: string): Promise<SupabaseAuthResult> {
    try {
      ...
      if (data.user) {
        console.log(
          "[SupabaseAuthAdapter] Signin successful for user:",
          data.user.id
        );
        return {
          success: true,
  ...
  async signUp(userData: {
    ...
    try {
      console.log(
        "[SupabaseAuthAdapter] Starting signup process for:",
        userData.email
      );
      ...
      if (data.user) {
        console.log(
          "[SupabaseAuthAdapter] Signup successful for user:",
          data.user.id
        );
        return {
          success: true,
  ...
  async getUserByEmail(email: string): Promise<SupabaseUser | null> {
    try {
      ...
      if (error && error.code !== "PGRST116") {
        ...
        console.error(
          "[SupabaseAuthAdapter] Error fetching user by email:",
          error
        );
        return null;
      }
      ...
      console.error(
        "[SupabaseAuthAdapter] Unexpected error fetching user by email:",
        error
      );
      return null;
    }
  ...
  async getCurrentUser(): Promise<SupabaseUser | null> {
    try {
      ...
      if (error) {
        console.error(
          "[SupabaseAuthAdapter] Error getting current user:",
          error
        );
        return null;
      }

      console.log("[SupabaseAuthAdapter] Current user retrieved:", {
        hasUser: !!user,
      });
      return user;
    } catch (error) {
      console.error(
        "[SupabaseAuthAdapter] Unexpected error getting current user:",
        error
      );
      return null;
    }
  ...
  async updateUserProfile(updates: {
    ...
    try {
      ...
      if (error) {
        console.error(
          "[SupabaseAuthAdapter] Failed to update user profile:",
          error
        );
        return { success: false, error: error.message };
      }
      ...
      console.error(
        "[SupabaseAuthAdapter] Unexpected error updating user profile:",
        error
      );
  ...
  async deleteUser(): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      console.error(
        "[SupabaseAuthAdapter] Unexpected error deleting user:",
        error
      );
    ...
    try {
      ...
      if (typeof window !== "undefined") {
        try {
          ...
          console.log(
            "[SupabaseAuthAdapter] Cleared Supabase localStorage items:",
            keysToRemove
          );
        } catch (error) {
          console.warn(
            "[SupabaseAuthAdapter] Failed to clear localStorage:",
            error
          );
        }
      }
  ...
  async getCurrentSession(): Promise<{
    ...
    try {
      ...
      console.log("[SupabaseAuthAdapter] Session retrieved:", {
  ...
  async refreshToken(refreshToken: string): Promise<{
    ...
    try {
      console.log(
        "[SupabaseAuthAdapter] Refreshing token with provided refresh token"
      );
      ...
      console.error(
        "[SupabaseAuthAdapter] Unexpected token refresh error:",
        error
      );
      const errorMessage =
  ...
  async requestPasswordReset(email: string): Promise<{
    ...
    try {
      console.log(
        "[SupabaseAuthAdapter] Requesting password reset for:",
        email
      );
      ...
      if (error) {
        console.error(
          "[SupabaseAuthAdapter] Password reset request error:",
          error
        );
      ...
      console.error(
        "[SupabaseAuthAdapter] Unexpected password reset request error:",
        error
      );
      const errorMessage =
  ...
  async resetPassword(
    ...
    try {
      ...
      console.error(
        "[SupabaseAuthAdapter] Unexpected password reset error:",
        error
      );
      const errorMessage =
