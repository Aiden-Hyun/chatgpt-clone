...
export type RoomModelState = { model: string; status: 'idle' | 'loading' | 'ready' | 'error' };
...
export const ModelStore = {
  ...
  set(key: RoomKey, next: RoomModelState): void {
    if (__DEV__) console.log('[ModelStore.set]', { key, next });
    store.set(key, next);
  ...
  subscribe(key: RoomKey, cb: () => void): () => void {
    if (__DEV__) console.log('[ModelStore.subscribe] add', { key });
    const set = listeners.get(key) ?? new Set<() => void>();
    set.add(cb);
    listeners.set(key, set);
    return () => {
      if (__DEV__) console.log('[ModelStore.subscribe] remove', { key });
  ...
  attachPendingToRoom(newRoomId: number): void {
    if (__DEV__) console.log('[ModelStore.attachPendingToRoom]', { newRoomId });
