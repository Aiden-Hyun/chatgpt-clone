...
export const ModelRepository = {
  async get(roomId: number): Promise<string | null> {
    try {
      if (__DEV__)
        console.log("[ModelRepository.get] fetch model for room", roomId);
      ...
      if (__DEV__)
        console.log("[ModelRepository.get] result", { roomId, model });
      return model;
    } catch (e) {
  ...
  async update(roomId: number, model: string): Promise<void> {
    try {
      if (__DEV__)
        console.log("[ModelRepository.update] update model", { roomId, model });
      await supabase.from("chatrooms").update({ model }).eq("id", roomId);
      if (__DEV__)
        console.log("[ModelRepository.update] done", { roomId, model });
    } catch (e) {
