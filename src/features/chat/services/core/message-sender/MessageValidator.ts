...
export class MessageValidator {
  ...
  validateRequest(
    ...
    this.logger.debug(
      `Starting validation for request ${requestId} with model ${request.model}`
    );
    ...
    if (isSearchMode) {
      ...
      if (!modelInfo?.capabilities.search) {
        ...
        this.logger.error(
          `Search validation failed for model ${model}: ${error}`
        );
    ...
    this.logger.debug(
      `User content validation passed (${userContent.length} characters)`
    );
    ...
    this.logger.debug(
      `Created message objects: user ${userMsg.id}, assistant ${assistantMsg.id} (${userContent.length} chars)`
    );
