...
export class MessageValidator {
  ...
  validateRequest(request: SendMessageRequest, requestId: string): ValidationResult {
    console.log('🔍 [MessageValidator] Starting validation for request:', requestId);
    
    const { userContent, model, isSearchMode } = request;

    // Validate search mode is supported for this model
    if (isSearchMode) {
      console.log('🔍 [MessageValidator] Checking search mode support for model:', model);
      ...
      console.log('✅ [MessageValidator] Search mode validation passed for model:', model);
    }

    // Validate user content
    if (!userContent || userContent.trim().length === 0) {
      const error = 'User content cannot be empty';
      console.error('❌ [MessageValidator] User content validation failed:', error);
      this.loggingService.error(`User content validation failed for request ${requestId}`, { error });
      return { isValid: false, error };
    }
    console.log('✅ [MessageValidator] User content validation passed, length:', userContent.length);
