...
export class MessageErrorHandler {
  ...
  handleError(context: ErrorContext): void {
    ...
    console.log('🚨 [MessageErrorHandler] Handling error for request:', requestId, {
      errorMessage,
      duration,
      assistantMessageId
    });
    ...
    if (assistantMessageId) {
      console.log('📝 [MessageErrorHandler] Marking specific message as error:', assistantMessageId);
      ...
      console.log('📝 [MessageErrorHandler] Marking last assistant loading message as error (fallback)');
  ...
  handleAIResponseError(requestId: string, assistantMessageId: string, error: string): void {
    console.log('🤖 [MessageErrorHandler] Handling AI response error for request:', requestId, {
      assistantMessageId,
      error
    });
    ...
    console.log('⌨️ [MessageErrorHandler] Clearing typing state due to AI response error');
    this.typingStateService.setTyping(false);
    
    console.log('✅ [MessageErrorHandler] AI response error handling completed');
  }
