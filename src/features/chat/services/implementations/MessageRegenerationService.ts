  ...
  async regenerateMessage(
    ...
    try {
      ...
      if (index < 0 || index >= messages.length) {
        console.error(
          "🔄 REGEN-SERVICE: Message not found for index:",
          index,
          "len:",
          messages.length
        );
      ...
      if (!target?.id) {
        console.error(
          "🔄 REGEN-SERVICE: Target message missing id at index:",
          index
        );
        throw new Error("Message not found for regeneration");
      }
      ...
      if (
        ...
        Math.floor(Date.now() / 1000) > this.session.expires_at
      ) {
        try {
      ...
      const apiResponse = await this.aiApiService.sendMessage(
        apiRequest,
        accessToken,
        this.isSearchMode
      );
